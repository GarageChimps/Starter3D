...
{
  float scaledU = (floor(input.fragTextureCoords.x * numU) / numU);
  float scaledV = (floor(input.fragTextureCoords.y * numV) / numV);
  float2 scaledCoords = float2(scaledU, scaledV);
    float3 diffuseColor = diffuseTexture.Sample(BilinearTextureSampler, scaledCoords);
