...
float3 BRDF(float3 normal, float3 lightDirection, float3 viewDirection, float3 halfVector, float3 diffuse, float3 specular)
{
  return lambertBRDF(normal, lightDirection, diffuse) + blinnPhongBRDF(normal, halfVector, specular, shininess);
}

float3 shade(float3 p, float3 n, float3 diffuse, float3 specular)
{
  n = normalize(n);
...
{  
  ...
  float3 specularColor = specularTexture.Sample(BilinearTextureSampler, input.fragTextureCoords.xy);
  return float4(shade(input.fragPosition, input.fragNormal, diffuseColor, specularColor), 1.0);
}
